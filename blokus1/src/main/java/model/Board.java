...
public class Board{
    ...
    public boolean isValidPiecePlacement(Piece piece, Square centerPiece, Player player) {
            ...
            for (int j = 0; j < shape[i].length; j++) {
                if (shape[i][j] == Shape.PIECE) {// non-empty square in the piece
                    ...
                    try {
                        ...
                        if ((board[r][c].getColorValue().get(player.getColor())== Shape.SIDE)) {
        ...
        if (isAdjaced) {
            ...
            throw new GameException("Invalid placement - piece need to touch another player's piece corner");
        }
    }
        ...
        for (int i = 0; i < shape.length; i++) {
            ...
            for (int j = 0; j < shape[i].length; j++) {
                ...
                if (!checkIfInBound(r,c))
                    continue;
                if (shape[i][j] != 0) {// non-empty square in the piece
...
//                    if (Board.checkIsCorner(row,col) && board[row][col].getColor() == BColor.WHITE) {
...
//                for (int col = 0; col < size; col++) {
//                    try {
//                        if (isValidPiecePlacement(piece,board[row][col],player)) {
//                            moves.add(board[row][col]);
//                        }
//                    }catch (GameException e){}
//
